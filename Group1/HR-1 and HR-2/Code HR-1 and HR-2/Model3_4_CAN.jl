# Paper
#Now we will introduce the cannibalism constraint into H-R1 and H-R2 of the following paper
...
function solucionar_modelo(instancia, output_instancia)
    let
        ...
        #println("m=$m, n=$n, R=$R")
        num_can = div(n,5) * 2
        CAN = Array{Int64}(undef,num_can)   #array to store the pairs os cannibal products
        #println("num_can = $num_can")
        for i=1 : m
            linha = readline(arq, keep=true)
            pieces = split(linha, ' ', keepempty=false)
            for j=1 : n
        ...
        #The real last with CANNIBALISM brings the pairs of cannibal products
        linha = readline(arq, keep=true)
        pieces = split(linha, ' ', keepempty=false)
        for j=1 : num_can
            CAN[j] = (parse(Int64, pieces[j]))+1
            #println("CAN[$j]= ",CAN[j])
        end
        close(arq)
        ...
        # MODELO
        modelo = Model(with_optimizer(Gurobi.Optimizer, Presolve=1))
        ...
        valor_fo = 0
        Ne = Int64[]
        # Frist of all I take out all the cannibal products following the rule:
        # take out the cannibal product with maximum X[m+1,j]/O[j] between each pairs
        for i = 1:2:num_can
            if MplusOne[CAN[i]] >= MplusOne[CAN[i+1]]
                k = CAN[i]
            else
                k = CAN[i+1]
            end
        ...
        while flag_of_infeasibility
            ...
            if (termination_status(model3) == MOI.OPTIMAL)
                ...
                valor_fo = objective_value(model3)
            else
        ...
        # take out the cannibal product with maximum X[m+1,j]/O[j] between each pairs
        for i = 1:2:num_can
            if foverB[CAN[i]] >= foverB[CAN[i+1]]
                k = CAN[i]
            else
                k = CAN[i+1]
            end
        ...
        while flag_of_infeasibility
            ...
            modelo4 = Model(with_optimizer(Gurobi.Optimizer, Presolve=1))
...
for (root, dirs, files) in walkdir("C:/Users/Greici da Rosa/Documents/JULIA/Instancias")
    for file in files
        ...
        output_instancia = joinpath("C:/Users/Greici da Rosa/Documents/JULIA/Resultado_Modelo3_4_CAN", basename(root))
